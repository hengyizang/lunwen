...
function renderMetrics(metrics) {
  const items = [
    ...
    ["模型费用", metrics.model_cost_cny == null ? "未配置" : `¥${Number(metrics.model_cost_cny).toFixed(2)}`],
    ["费用上限", metrics.model_budget_cny == null ? "未配置" : `¥${Number(metrics.model_budget_cny).toFixed(2)}`],
    ["剩余额度", metrics.model_budget_remaining_cny == null ? "未配置" : `¥${Number(metrics.model_budget_remaining_cny).toFixed(2)}`],
