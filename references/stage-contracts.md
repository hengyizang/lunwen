...
- passing, current `reviews/revision-trace.json` proving promised response-matrix changes are present, and `reviews/revision-integrity.json` proving numeric, citation and claim-strength changes match named human authorization;
...
- claim-forward result placement: the abstract and conclusion prioritize the preregistered primary finding, while every registered outcome remains locatable in the manuscript or supplement. Main-text versus supplement placement must follow predeclared role, relevance and statistical adequacy rather than result direction; primary outcomes, falsification tests, required baselines, material robustness failures and interpretation-changing results stay visible in the main text. Material limitations are stated once where they change interpretation, without repeated apologetic caveats.
- an applicable reporting-guideline checklist routed from `references/reporting-guidelines.md`, with manuscript locations and explicit reasons for genuinely non-applicable items.
- for newly authored papers, a fact-location and measurement/argument ledger checked with `scripts/manuscript_audit.py`; investigate any unresolved issue rather than forcing a superficial lexical pass. External review letters, if present, must have concern cards and a venue-budgeted response that covers every comment.

Result-bearing charts still require deterministic data/run provenance. An optional conceptual illustration from GPT Image or a user-operated ChatGPT web session is separate: the source/prompt hash, claimed model ID, named visual inspection and AI-use disclosure location must be registered before it can appear in the package. Such imagery cannot substantiate a result, replace a method/data chart or hide uncertainty.
