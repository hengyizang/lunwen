...
- `papers/Pxx/power-analysis.json` from pinned `statsmodels` or a hash-bound Monte Carlo script/result with at least 1,000 simulations; narrative claims of adequate power do not count;
...
- a reproduction plan in every design mapping approved run IDs to domain-standard and strong-recent baselines and to disjoint original/clean-room executions, with different approved checkouts, environment capture, an independent-operator plan and predeclared numeric tolerances.
...
- `papers/Pxx/clean-room-reproduction.json` for every paper, using the exact G3-declared disjoint run IDs, exact successful attempt IDs, different recorded checkout paths, identifier-independent registry environment digests, predeclared numeric equivalence checks and the complete current output set;
