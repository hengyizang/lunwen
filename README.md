...
- G3 对每个入选数据集生成本地哈希绑定质量报告，并要求每篇论文有 `statsmodels` 或至少 1,000 次 Monte Carlo 的可执行功效证据；全部设计、数据、预算和功效报告由人确认后冻结为预注册，之后任何改动都会使闸门失效。
...
- G4 每篇论文必须复现领域标准和强近期基线，并用不同运行 ID、独立 checkout/环境记录完成隔离复现；容差会按数值重新计算，运行、环境与证据哈希来自控制层生成的 catalog，不能由模型自报。
...
- 新颖性矩阵、数据质量及具名哈希确认、统计功效、预注册、强基线复现和隔离复现状态；
- 六篇论文的顺序状态和本地投稿 ZIP。

完整说明、安全边界和故障处理见 [`docs/DASHBOARD.md`](docs/DASHBOARD.md)。CLI 仍然保留，并与客户端共享完全相同的项目状态。

新颖性、数据质量、可执行功效、预注册、基线复现和隔离复现的完整操作见 [`docs/RESEARCH-QUALITY.md`](docs/RESEARCH-QUALITY.md)。
