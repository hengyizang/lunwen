...
Inspect and ingest the authorized local template with `scripts/venue_adapter.py`. Never overwrite the publisher sample; create a new manuscript entry file through Codex/OpenAI, not Claude. Map the venue-neutral semantic manuscript into the official template, then run `scripts/venue_compliance.py` to recheck inventory hashes, manuscript basics and a no-shell-escape LaTeX build when available. Inspect the resulting PDF/DOCX manually.
