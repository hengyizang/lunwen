...
DIMENSIONS = {
    "G1": (
        "novelty", "doctoral_depth", "significance", "feasibility",
        "evidence_strength", "publication_potential"
    ),
    "G2": (
        "novelty", "distinct_contribution", "significance", "feasibility",
        "methodological_rigor", "q1_fit"
    ),
    "G3": (
        "novelty", "feasibility", "methodological_rigor", "statistical_rigor",
        "data_quality", "reproducibility", "q1_fit"
    ),
    "G4": (
        "novelty_supported", "effect_robustness", "statistical_rigor",
        "reproducibility", "claim_evidence_strength", "q1_fit"
    ),
    "G5": (
        "novelty", "scientific_contribution", "methodological_rigor",
        "evidence_strength", "q1_fit", "review_resilience", "writing_quality"
    ),
}
...
def validate_report(report: dict[str, Any], gate: str) -> list[str]:
    ...
    if report.get("status") not in {"ready_for_review", "approved"}:
        errors.append("status must be ready_for_review or approved")
    dimensions = report.get("scores")
    if not isinstance(dimensions, dict):
        errors.append("scores object is required")
        dimensions = {}
    scores: list[float] = []
    for field in DIMENSIONS.get(gate, ()):
        ...
        if value is not None:
            scores.append(value)
    if scores and sum(scores) / len(scores) < MIN_OVERALL:
        errors.append(f"overall mean score must be >= {MIN_OVERALL:g}")
        ...
        for index, item in enumerate(evidence, 1):
            ...
            for field in ("id", "source", "claim_supported", "source_date"):
                if not str(item.get(field, "")).strip():
                    errors.append(f"evidence[{index}] needs {field}")
    ...
    if report.get("novelty_is_absence_only") is not False:
        errors.append("novelty_is_absence_only must be false")

    q1 = report.get("q1_target")
    if not isinstance(q1, dict):
        errors.append("q1_target object is required")
    else:
        candidates = q1.get("candidate_venues")
        if not isinstance(candidates, list) or len(candidates) < 2:
            errors.append("at least two Q1 candidate venues are required")
        if q1.get("current_verification_required") is not True:
            errors.append("current_verification_required must be true")
        for index, candidate in enumerate(candidates or [], 1):
            if not isinstance(candidate, dict):
                errors.append(f"q1_target.candidate_venues[{index}] must be an object")
                continue
            if candidate.get("quartile") not in Q1_LABELS:
                errors.append(f"q1 candidate {index} must be explicitly Q1")
            if candidate.get("indexing") not in {"SCIE", "SCI"}:
                errors.append(f"q1 candidate {index} must be SCI/SCIE")
            if not str(candidate.get("source_url", "")).startswith("https://"):
                errors.append(f"q1 candidate {index} needs an HTTPS primary source")

    blockers = report.get("blockers")
    if not isinstance(blockers, list):
        errors.append("blockers must be a list")
    elif blockers:
        errors.append("quality report has unresolved blockers")
...
def validate_file(path: Path, gate: str) -> list[str]:
    try:
        ...
        return [f"missing quality report: {path}"]
    except json.JSONDecodeError as exc:
        return [f"invalid JSON in quality report {path}: {exc}"]
    if not isinstance(report, dict):
        return [f"quality report must be an object: {path}"]
