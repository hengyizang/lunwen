#!/usr/bin/env python3
"""Record a human-verified, recent paper-specific JCR Q1/Q2 venue check."""
...
def verify(payload, required_quartile="Q1"):
    ...
    required=normalize_quartile(required_quartile)
    if required is None:raise JcrVerificationError("required quartile must be Q1 or Q2")
    if not meets_target(payload["quartile"],required):raise JcrVerificationError(f"quartile must meet the current JCR {required} paper target")
...
def main():
    ...
    target="Q1";contract=a.paper_dir/"paper-contract.json"
    if contract.is_file():
        try:
            from scripts.venue_policy import target_from_contract
        except ImportError:
            from venue_policy import target_from_contract
        try:target=target_from_contract(json.loads(contract.read_text(encoding="utf-8")))
        except (OSError,json.JSONDecodeError):pass
    verify(payload,target);a.paper_dir.mkdir(parents=True,exist_ok=True);out=a.paper_dir/"jcr-verification.json";out.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+"\n",encoding="utf-8");print(out);return 0
